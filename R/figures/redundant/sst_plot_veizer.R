...
p2 <- ggplot() +
  ...
  geom_line(data = stage_vals, mapping=aes(x = mid_ma, y = temperature_stage), colour = "black", size = 1.1, alpha  = 1) +
  scale_x_reverse(expand=c(0,0), limits = c(541, 0)) +
  scale_y_continuous(expand=c(0,0), limits = c(-6, 45)) +
  labs(x = "", y = expression(bold(paste("Temperature (",degree, C,")"))), subtitle = "Extracted Modern") +
  theme(panel.background = element_blank(),
        plot.margin = margin(0.25,0.5,0.25,0.5, "cm"),
        legend.position = c(0.2, 0.9),
        legend.background = element_blank(),
...
p3 <- ggplot() +
  geom_rect(data = throwing_shade, mapping=aes(xmin=min_ma, xmax=max_ma, ymin = 10, ymax= Inf), linetype = 0, color="grey90", alpha=0.1)  +
  geom_rect(data = periods, mapping=aes(xmin = min_ma, xmax = max_ma, ymin = 7, ymax = 10), linetype = 1, colour = "black", fill=periods$color, alpha=1)  +
  geom_text(data = periods, mapping=aes(x = mid_ma, y = 8.5, label = abbrev), colour = "black", alpha=1)  +
  ...
  geom_line(data = stage_vals, mapping=aes(x = mid_ma, y = temperature_stage), colour = "black", size = 1.1, alpha  = 1) +
  scale_x_reverse(expand=c(0,0), limits = c(541, 0)) +
  scale_y_continuous(expand=c(0,0), limits = c(7, 45)) +
...
plots <- ggarrange(p1, p2, p3, ncol=1, nrow=3, labels = "AUTO", align = "v")

ggsave(file = "./figures/extracted_temperature.png", plot = plots, width = 150, height = 215, units = "mm", dpi = 300, scale = 1.15)
