...
eoc_dat <- read.csv("./data/zhang_et_al_2019_eocene.csv")
...
p <- ggplot() +
  ...
  geom_point(data = eoc_dat, mapping=aes(x = abs(paleolat), y = MAT, colour="#084594"), size = 1.25, alpha=0.7) +
  scale_color_manual(values = c("#b10026","#084594"), labels = c("Eocene", "Modern")) +
  #scale_x_reverse(expand=c(0,0), limits = c(0, 95)) +
  labs(x = expression(bold(paste("Absolute latitude (",degree,")"))), y = expression(bold(paste("Temperature (",degree, C,")")))) +
