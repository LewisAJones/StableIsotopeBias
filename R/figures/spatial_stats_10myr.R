...
#---------------------------------
#read data
data <- read.csv("./results/spatial_stats/myr10_spatial_stats.csv")
...
plot_a <- ggplot() +
  ...
  geom_ribbon(data = data, mapping = aes(x = age, y = Median, ymin = Min, ymax = Max), colour = NA, fill = col[1], size = 0.75, alpha = 0.3) +
  geom_ribbon(data = data, mapping = aes(x = age, y = Median, ymin = Lower, ymax = Upper), colour = NA, fill = col[1], size = 0.75, alpha = 0.3) +
  geom_line(data = data, mapping=aes(x = age, y = Median), colour = col[1], size = 1.1, alpha  = 0.75) +
  #geom_errorbar(data = data, mapping=aes(x = mid_ma, ymin = Lower, ymax= Upper), width=0, position=position_dodge(0.2)) +
  geom_point(data = data, mapping=aes(x = age, y = Median), colour = col[1], size = 2, alpha  = 0.75) +
...
plot_b <- ggplot() +
  ...
  geom_rect(data = throwing_shade, mapping=aes(xmin=min_ma, xmax=max_ma, ymin = -10, ymax= 120), linetype = 0, color="grey90", alpha=0.1)  +
  #geom_rect(data = periods, mapping=aes(xmin=300, xmax=0, ymin= -3, ymax= 0), linetype = 1, colour = "black", fill="black", alpha=1)  +
  geom_rect(data = periods, mapping=aes(xmin = min_ma, xmax = max_ma, ymin = -10, ymax = 0), linetype = 1, colour = "black", fill=periods$color, alpha=1)  +
  geom_text(data = periods, mapping=aes(x = mid_ma, y = -5, label = abbrev), size = 4, colour = "black", alpha=1)  +
  geom_ribbon(data = data, mapping = aes(x = age, ymin = 0, ymax = occupancy), colour = NA, fill = "grey40") +
  geom_ribbon(data = data, mapping = aes(x = age, ymin = 0, ymax = ODP), colour = NA, fill = "black") +
  geom_text(data = NULL, mapping=aes(x = 350, y = 110, label = "Non-ocean drilling sites"), fontface = "bold", size = 4, colour = "grey40", alpha=1)  +
  geom_text(data = NULL, mapping=aes(x = 72, y = 110, label = "Ocean drilling sites"), fontface = "bold", size = 4, colour = "black", alpha=1)  +
  #geom_line(data = data, mapping=aes(x = mid_ma, y = occupancy), colour = col[1], size = 1.1, alpha  = 0.75) +
  scale_x_reverse(expand=c(0,0), limits = c(541, 0)) +
  scale_y_continuous(expand=c(0,0), limits = c(-10, 120)) +
...
plot_c <- ggplot() +
  ...
  geom_rect(data = throwing_shade, mapping=aes(xmin=min_ma, xmax=max_ma, ymin = -9, ymax= Inf), linetype = 0, color="grey90", alpha=0.1)  +
  #geom_rect(data = periods, mapping=aes(xmin=300, xmax=0, ymin= -2600, ymax= 0), linetype = 1, colour = "black", fill="black", alpha=1)  +
  geom_rect(data = periods, mapping=aes(xmin = min_ma, xmax = max_ma, ymin = -9, ymax = 0), linetype = 1, colour = "black", fill=periods$color, alpha=1)  +
  geom_text(data = periods, mapping=aes(x = mid_ma, y = -4.5, label = abbrev), size = 4, colour = "black", alpha=1)  +
  geom_line(data = data, mapping=aes(x = age, y = MST), colour = col[1], size = 1.1, alpha  = 0.75) +
  geom_point(data = data, mapping=aes(x = age, y = MST), colour = col[1], size = 2, alpha  = 0.75) +
  scale_x_reverse(expand=c(0,0), limits = c(541, 0)) +
  scale_y_continuous(expand=c(0,0), limits = c(-9, 100)) +
...
#plots
ggsave(file = "./figures/spatial_stats_10myr.jpg", plot = plots, width = 117, height = 180, units = "mm", dpi = 600, scale = 1.6)